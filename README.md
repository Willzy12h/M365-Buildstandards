...
**1.1.0-preview.8 — implementation preview, not tenant-accepted.** Standard 2026.09.8 has 53 controls: 45 candidate recipes (including the 8 directory prerequisites every other control depends on), 4 controls assessed from captured evidence (authentication methods, MDM scope, tenant compliance default, administrator access), and 4 that remain manual by nature (emergency accounts, Apple and Google ownership, Autopatch). Policy automation now includes input/import UI, saved tenant-local candidate standards, Entra LAPS, reviewed tenant settings and activation/assignment, Autopatch device enrolment, Win32 package publishing and readiness checks. Directory prerequisites are created empty and untrusted; membership and trust remain deliberate engineering steps. See [automation coverage](docs/AUTOMATION-COVERAGE.md), [handover](docs/integration/HANDOVER.md) and [testing evidence](docs/integration/TESTING-EVIDENCE.md).
