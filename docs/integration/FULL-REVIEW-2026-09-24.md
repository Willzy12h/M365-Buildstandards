...
- **Six pages hid content at small sizes.** Assessment, Configuration, Deviations, Manual checks, Plan and Build Standard give their tables whatever height is left under the heading, with no page scroll. At 1180x640 the Plan and Build Standard guidance was cut off with nothing to scroll to. These pages now fill the window down to `PageLayout.MinimumHeight` (600) and scroll below it; at the default size they are well above it and nothing changes.
