...
Before running any historical checklist below, use the [bounded F1–F5 acceptance proposal](https://github.com/Willzy12h/M365-Buildstandards/blob/integration/docs/integration/SAFETY-REVIEW-2026-09-21.md#bounded-acceptance-proposal--not-authorisation) (development record, opens on GitHub) and obtain separate human approval of exact tenant/identities/populations/exclusions/recovery limits. Current source approval is not tenant authorisation. Generic Autopilot group assignment/removal is blocked. User/device exclusion compatibility must be recorded explicitly; GUID resolution is not effectiveness. Windows Hello omitted-character defaults remain unverified and require device-level acceptance rather than JSON-only claims.
