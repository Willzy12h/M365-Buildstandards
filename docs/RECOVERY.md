...
Safety follow-up: generic Autopilot assignment removal is **not supported** and must not be treated as containment. Enrolment removal uses the resource-specific reviewed envelope. Accepted beta EDR restoration can be re-verified using the collection's beta API and reads only; v1.0 EDR remains rejected. These are synthetic-tested code contracts, not live recovery acceptance. See [F1–F5 limits](https://github.com/Willzy12h/M365-Buildstandards/blob/integration/docs/integration/SAFETY-REVIEW-2026-09-21.md) (development record, opens on GitHub).
...
The complete-capture requirement can prevent toolkit recovery when unrelated tenant reads fail. Use an independently authorised Entra/Intune recovery procedure in that situation; the toolkit does not bypass its evidence gate. Policy automation now offers separately reviewed removal of all assignments from a toolkit-created Intune candidate, followed by a fresh capture/recovery preview. Enterprise-app deletion, consent revocation and engineer app-access assignment removal remain separate administrative procedures. App setup records application IDs, service-principal IDs and write outcomes; deleting the authentication applications could prevent the toolkit from continuing.

Preview.6 extends owned, unassigned candidate deletion to enrolment profiles, Settings Catalogue, mobile app protection and application objects, using each collection's declared API version. After an encrypted package upload, deleting its recorded app is the available cleanup path; publishing upgrades over committed content is blocked. Tenant-wide compliance/authentication/MDM changes and device effects have no automatic rollback. See [automation coverage](AUTOMATION-COVERAGE.md) for containment and re-verification limits.
...
Synthetic tests cover durable intent, exact IDs, deletion and absence, update restoration, unexpected activation, assignment/drift/tenant/approval gates, uncertain writes and shared-store exclusion. Real tenant recovery remains unverified; see [testing evidence](https://github.com/Willzy12h/M365-Buildstandards/blob/integration/docs/integration/TESTING-EVIDENCE.md) (development record, opens on GitHub).
